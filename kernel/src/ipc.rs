...
///
/// This enum represents the "System Call" interface or IPC protocol.
/// Each variant corresponds to a specific action or piece of data.
#[derive(Clone, Copy)]
pub enum Message {
    /// A single character to be printed to the screen.
    /// Payload: The ASCII byte.
    VgaPrint(u8),
    /// A command to clear the entire screen.
    VgaClear,
    /// A command to move the cursor back one position (backspace).
    VgaBackspace,
    /// A command to move the cursor to the next line.
    VgaNewline,
    /// Raw keyboard event data.
    /// Payload: The specific key event.
    KeyboardEvent(KeyEvent),
}

/// Key events generated by input devices (e.g., keyboard).
///
/// These are lower-level than characters; they distinguish between
/// printable keys, modifiers, and control keys.
#[derive(Clone, Copy)]
pub enum KeyEvent {
